...
**Important Slack Configuration Note:** By default, Hermes Agent requires explicit @mention to respond in Slack channels (slack.require_mention: true). If your bot appears online but doesn't respond to channel messages, check this setting first. To allow responses without mention:
- Disable globally: `hermes config set slack.require_mention false`
- Allow specific channels: `hermes config set slack.free_response_channels "general,random"`
...
**Slack-specific setup:** Hermes Agent's Slack integration uses Socket Mode and requires two tokens:\n1. **SLACK_BOT_TOKEN** (xoxb-...) - from OAuth & Permissions → Bot Token Scopes\n2. **SLACK_APP_TOKEN** (xapp-...) - from Socket Mode settings (toggle ON, generate with `connections:write` scope)\n\nPlatform docs: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/\nSee Slack platform adapter details: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/#slack\n\nFor detailed troubleshooting of Slack Socket Mode connection issues, see `references/slack-troubleshooting.md`.

**Important Configuration Note:** By default, Hermes Agent requires explicit @mention to respond in Slack channels (slack.require_mention: true). If your bot appears online but doesn't respond to channel messages, check this setting first. To allow responses without mention:\n- Disable globally: `hermes config set slack.require_mention false`\n- Allow specific channels: `hermes config set slack.free_response_channels "general,random"`\nThen restart the gateway.

**Quick Troubleshooting Slack Socket Mode:**
- If the gateway exits immediately after starting, check the logs for `[Slack] SLACK_APP_TOKEN not set` or `[Slack] SLACK_BOT_TOKEN not set`.
- Ensure both tokens are present in your `.env` file (or environment variables) and that the Slack app has Socket Mode enabled and the required scopes.
...
- **Slack Socket Mode connection fails**: Requires both `SLACK_BOT_TOKEN` (xoxb-...) and `SLACK_APP_TOKEN` (xapp-...). Check logs for "[Slack] SLACK_APP_TOKEN not set" or "[Slack] SLACK_BOT_TOKEN not set".
