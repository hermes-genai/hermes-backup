...
> **Important Auth Note**: Most X API v2 endpoints require **OAuth 2.0 User Context** (not Application-Only auth). When using raw API access with a Bearer Token, you'll get an "Unsupported Authentication" error for endpoints like `/2/users/me`. For these endpoints, you must complete the OAuth 2.0 flow (`xurl auth oauth2`) and use the resulting user context token. Application-Only auth (Bearer Token) only works for a limited set of endpoints.
